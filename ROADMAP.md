...
| 0.2 | Install dependencies: Tailwind, Recharts, Leaflet, esbuild, Supabase client | All | ⬜ | One person runs, others watch for errors |
| 0.3 | Create Supabase project, copy URL + anon key into `.env.local` | All | ⬜ | Free tier at supabase.com |
| 0.4 | Run all 4 SQL schema migrations in Supabase SQL editor (sites, events, hourly_stats, funnels) | All | ⬜ | Copy from PRD Section 4 |
...
| 0.8 | Add Anthropic API key to `.env.local` and Vercel environment variables | All | ⬜ | `ANTHROPIC_API_KEY=sk-...` |

**Phase 0 Exit Criteria:** Supabase tables exist, Next.js runs locally, Vercel deploys successfully.
...
| # | Task | Owner | Status | Notes |
|---|---|---|---|---|
| 1B.1 | Create Supabase server client utility (`/lib/supabase-server.ts`) | B | ⬜ | Uses service role key for server-side writes |
...
| 1B.10 | Test dual-write: send 5 test events, confirm both `events` and `hourly_stats` rows exist in Supabase | B | ⬜ | Check Supabase table editor |
...
| # | Task | Owner | Status | Notes |
|---|---|---|---|---|
| 1C.1 | Set up Supabase Auth — email/password login | C | ⬜ | Use Supabase pre-built auth helpers for Next.js |
...
**Phase 1 Exit Criteria:** Events flow from browser → API → Supabase. Dashboard shell renders with auth. Stats API returns data.
...
| 2A.3 | Add velocity check — track requests per IP in a server-side `Map` with 10s TTL | A | ⬜ | If > 10 events in 10s → isBot = true |
...
| 2A.8 | Set up Supabase Realtime subscription on `events` table for the Live Feed | A | ⬜ | Filter by `site_id` |
...
| 2B.10 | Run seed script against Supabase, verify `hourly_stats` aggregates correctly | B | ⬜ | |
...
| 2C.8 | Create `POST /api/signals` route — calls Claude API with hourly stats, returns insight array | C | ⬜ | See PRD Section 8 for full prompt |
...
| # | Task | Owner | Status | Notes |
|---|---|---|---|---|
| 3A.1 | Create `POST /api/narratives` route — takes session_hash, fetches events, calls Claude | A | ⬜ | See PRD Section 8 for prompt template |
| 3A.2 | Format event sequence as plain-text journey string for Claude prompt | A | ⬜ | "Visited / → Clicked Pricing → Visited /checkout → Dropped" |
| 3A.3 | Include device (mobile/desktop from UA) and country in Claude prompt | A | ⬜ | Parse UA server-side |
...
| 3B.6 | Poll `GET /api/active-locations` every 10 seconds — update markers | B | ⬜ | Or use Supabase Realtime push |
...
| # | Task | Owner | Status | Notes |
|---|---|---|---|---|
| 3C.1 | Add Heatmap AI Opinion: after canvas renders, call Claude with click distribution summary | C | ⬜ | See PRD Section 5.6 for prompt |
...
| # | Task | Owner | Status | Notes |
|---|---|---|---|---|
| 4.1 | Final Vercel deploy — confirm all env vars set in production | All | ⬜ | Check: SUPABASE_URL, SUPABASE_ANON_KEY, ANTHROPIC_API_KEY |
| 4.2 | Run tracker.js on demo site in production — generate real events | All | ⬜ | Click around all 4 pages multiple times |
| 4.3 | Verify Signal Cards load in production (real Claude API call) | All | ⬜ | |
...
| **Dashboard** | Live event feed (Realtime) | ⬜ |
